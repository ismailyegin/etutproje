    ...
    if basvuru.status!=PreRegistration.APPROVED:
        if not (User.objects.filter(email=basvuru.email).exists()):
            ...
            try:
                ...
                html_content = html_content + '<p> <strong>Site adresi:</strong> <a href="https://sbs.halter.gov.tr/newpassword?query=' + str(
                    fdk.uuid) + '">https://sbs.halter.gov.tr/sbs/profil-guncelle/?query=' + str(fdk.uuid) + '</p></a>'
